...
impl Error {
    fn is_incomplete_message(&self) -> bool {
        match self {
            Self::Proto(err) => err.is_incomplete_message(),
            _ => false,
        }
    }
    ...
    fn is_parse_status(&self) -> bool {
        match self {
            Self::Proto(err) => err.is_parse_status(),
            _ => false,
        }
    }
}
    ...
    (
        ...
        let res = async move {
            ...
            impl Builder {
                ...
                #[allow(unused)]
                fn set_host(mut self, val: bool) -> Self {
                    self.set_host = val;
                    self
                }
                ...
                fn http09_responses(mut self, val: bool) -> Self {
                    ...
                    self.inner = self.inner.http09_responses(val);
                    self
                }
            }
            ...
            $(builder.inner = builder.inner$(.$c_opt_prop($c_opt_val))*;)?
            ...
            *req.uri_mut() = builder.build().unwrap();
            ...
            let mut resp = sender.try_send_request(req).await.map_err(|e| e.into_error())?;
...
mod conn {
    ...
    async fn get() {
        ...
        let client = async move {
            ...
            let req = Request::builder()
                ...
                .unwrap();
            let mut res = client.try_send_request(req).await.expect("send_request");
    ...
    fn incoming_content_length() {
        ...
        let (mut client, conn) = rt
            .block_on(conn::http1::Builder::default().handshake(TokioIo::new(tcp)))
            .unwrap();
    ...
    fn aborted_body_isnt_completed() {
        ...
        let req = Request::builder()
            ...
            .unwrap();
        let res = client.try_send_request(req);
    ...
    fn pipeline() {
        ...
        let req = Request::builder()
            ...
            .body(Empty::<Bytes>::new())
            .unwrap();
        let res2 = client.try_send_request(req).map(|result| {
            let err = result.expect_err("res2").into_error();
    ...
    fn upgrade() {
        ...
        let (mut client, mut conn) = rt
            .block_on(conn::http1::Builder::default().handshake(TokioIo::new(io)))
            .unwrap();
    ...
    async fn client_100_then_http09() {
        ...
        let (mut client, conn) = conn::http1::Builder::default()
            .options(Http1Options::builder().http09_responses(true).build())
            .handshake(TokioIo::new(tcp))
            .await
            .unwrap();
    ...
    async fn test_try_send_request() {
        ...
        tokio::spawn(async move {
            ...
            let (mut client, mut conn) = conn::http1::Builder::default()
    ...
    async fn http2_detect_conn_eof() {
        ...
        client.try_send_request(req).await.expect("req1 send");
    ...
    async fn http2_keep_alive_detects_unresponsive_server() {
        ...
        let (_client, conn) = conn::http2::Builder::new(rt::TokioExecutor::new())
            ...
            .handshake::<_, hyper::body::Incoming>(TokioIo::new(io))
            .await
            .expect("http handshake");
    ...
    async fn http2_responds_before_consuming_request_body_no_trailers() {
        // Test that a early-response from server works correctly (request body wasn't fully
        // consumed). https://github.com/hyperium/hyper/issues/2872
        use hyper::service::service_fn;
        ...
        let resp = client.try_send_request(req).await.expect("send_request");
    ...
    async fn test_body_panics() {
        ...
        let error = client.try_send_request(req).await.unwrap_err().into_error();
    ...
    async fn http1_half_closed_peer_with_open_request_body_does_not_spin() {
        ...
        let (mut client, conn) = conn::http1::Builder::default()
            .handshake::<_, StreamBody<mpsc::Receiver<Result<Frame<Bytes>, io::Error>>>>(
                TokioIo::new(io),
            )
            .await
            .expect("handshake");
        ...
        let response_task = tokio::spawn(async move {
            let _ = client.try_send_request(req).await;
        });
    ...
    async fn h2_idle_stream_does_not_pin_connection_window() {
        ...
        let body_a = BodyExt::boxed(StreamBody::new(rx_a));
        let req_a = Request::post("http://localhost/a").body(body_a).unwrap();
        let mut client_a = client.clone();
        let a_handle = tokio::spawn(async move { client_a.try_send_request(req_a).await });
